...
const AUDIO_ELEMENT_ID = makeid(10);
...
interface AudioPlayerProps {
  ...
  /**
   * Enables the shortcuts, space for play/pause and arrow keys for seeking) if set to `true`. Defaults to `false`
   */
      ...
      if (event.ctrlKey) {
        switch (event.key) {
          ...
          case "ArrowUp": // Up arrow key
            adjustVolume(internalVolume + 0.1); // Increase volume by 0.1
            break;
          case "ArrowDown": // Down arrow key
            adjustVolume(internalVolume - 0.1); // Decrease volume by 0.1
            break;
          default:
            break;
        }
      ...
      {loading ? (
        ...
        >
          ...
          >
            ...
            <Timer
              seconds={
                minimal && isPlaying ? currentTime : Math.round(duration)
              }
            />
          </div>
          {/* Controls */}
